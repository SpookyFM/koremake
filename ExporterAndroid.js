...
ExporterAndroid.prototype.exportSolution = function (solution, from, to, platform, vr) {
  ...
  if (vr == "cardboard") {
    Files.copy(Paths.executableDir().resolve(Paths.get("Data", "android", ".classpath.Cardboard")), to.resolve(".classpath"), true);
	} else {
...
if (vr == "gearvr") {
  Files.copy(Paths.executableDir().resolve(Paths.get("Data", "android", "AndroidManifest.GearVr.xml")), to.resolve("AndroidManifest.xml"), true);
  	Files.copy(Paths.executableDir().resolve(Paths.get("Data", "android", "project.GearVr.properties")), to.resolve("project.properties"), true);
} else if (vr == "cardboard") {
  Files.copy(Paths.executableDir().resolve(Paths.get("Data", "android", "AndroidManifest.Cardboard.xml")), to.resolve("AndroidManifest.xml"), true);
  	Files.copy(Paths.executableDir().resolve(Paths.get("Data", "android", "project.Cardboard.properties")), to.resolve("project.properties"), true);
} else {
...
*/
  ...
  if (vr == "gearvr") this.p("# use += instead of := when defining the following variables: LOCAL_LDLIBS, LOCAL_CFLAGS, LOCAL_C_INCLUDES, LOCAL_STATIC_LIBRARIES");
    ...
    this.p("LOCAL_LDLIBS    += -llog -lGLESv3 -lOpenMAXAL -landroid");
    this.p("LOCAL_CPPFLAGS := -DVR_GEAR_VR -DHXCPP_ANDROID_PLATFORM=21");
  } else { 
    this.p("LOCAL_C_INCLUDES := " + includes);
    this.p("LOCAL_LDLIBS    := -llog -lGLESv2 -lOpenMAXAL -landroid");
   }
   if (vr == "cardboard") {
    this.p("LOCAL_CPPFLAGS := -DVR_CARDBOARD -D");
  }
	this.p();
